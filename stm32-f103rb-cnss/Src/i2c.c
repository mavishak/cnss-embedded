...
static uint8_t byte1_buff; //the address of the buffer= where we want to store the data we receive(short explanation on the reference manual p. 277)
...
void init_i2c1(void){
	...
	/*Enable ACK*/
	//I2C1->CR1 |= 0x0400; //enable after wards if needed
...
void I2C_write(uint8_t data){
	...
	while((I2C1->SR1 & 0x0080) == 0x0080); //TXE bit
...
/*Case of single byte to be received*/
uint8_t I2C_read_nack(){

	/*[reference manual p. 763]*/

	/*In ADDR event clear ACK bit*/
	I2C1->CR1 &= ~(0x0400);
